...
document.getElementById('saveButton').addEventListener('click', function () {
  ...
  let favoriteArtist=$("#favoriteAuthor").val();
...
async function checkCurrentPassword() {
  ...
  $.ajax({
    ...
    success:function(response){
      // alert("통신 성공! : "+response); 
      let result=response.exists;
      // alert("조회 결과 : "+result);
      checkCurrentPasswordMessage(result==="true");
    },
    error:function(request,e){
      alert("오류 발생 : "+requestAnimationFrame.status+"\n메시지 : "+requestAnimationFrame.responseText+"\n오류 : "+e);
    }
  });
}
